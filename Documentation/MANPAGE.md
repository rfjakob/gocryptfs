...
Note that `-masterkey=init` only reuses an *explicit* master key that was passed
to the `-init` phase. If the master key was randomly generated by `-init` (no
`-masterkey` given), there is nothing to carry over and the mount phase instead
relies on the password carried over from `-init`.
