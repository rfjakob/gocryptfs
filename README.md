...
An explicit master key passed to `-init` can be reused for the mount with
`-masterkey=init` in the mount phase; if the key was randomly generated by
`-init`, the password carry-over is used instead.
